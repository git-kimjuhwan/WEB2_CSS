...
-03/04
        ...
        <strong>태그 심각함 긴급함 강조할때 사용(굵은 텍스트)
        <em> 태그 강조된 텍스트를 표현할 때 사용.
        <mark>태그 형광펜으로 칠한것 처럼 텍스트를 정의할 때 사용.
        <sub>태그  아랫첨자 텍스트를 표현할 때 사용 
        ex) 화학 반응식
        <del>태그 텍스트 한가운데 라인을 추가하여 문서에서 삭제된 텍스트를 표현할 때 사용함 
        <ins>태그 텍스트 아래쪽에 라인을 추가하여 문서에서 추가된 텍스트를 표현합니다.
        <q>태그 따옴표를 추가하기 위함.
...
-03/08 
        ...
        복습
        Border 는 테두리 
        style을 붙이면 테두리 종류 설정
        width를 붙이면 테두리 너비 설정 
        color를 붙이면 테두리 색상 설정

        
        CSS에서는 글씨에 밑줄이나 윗줄,중간줄을 넣을 때 text-decoration속성을 사용한다.

        test-decoration 속성값

        none 기본값,보통의 글자
        underline 글자아래에 밑줄
        overline 글자위에 줄이 그어짐  
...
-03/09
        ...
        #기호는 HTML id 송성을 CSS 선택자로 지정할 때 id명 앞에 붙이는 기호입니다. 
        .#기호는 html class 속성을 css선택자로 지정할 때 class명 앞에 붙이는 기호입니다.
        <link> 태그의 rel속성은 현재 문서와 외부 리소스 사이의 연관 관계를 명시합니다.
